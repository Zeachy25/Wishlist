...
import { CircularBuffer, calculateAnomaly, filterByTimeWindow } from '../algorithms/priceProcessor';
...
export const checkPriceDrops = async (userId: string): Promise<number> => {
  ...
  try {
    ...
    for (const item of wishlist) {
      ...
      const snapshots = history.map((h) => ({ price: Number(h.price), timestamp: h.timestamp }));
      ...
      const { isAnomaly, zScore } = calculateAnomaly(buffer.data, currentPrice, 1.5);
      ...
      if (isAnomaly) {
        ...
        if (!existing || existing.length === 0) {
          const alertPayload = {
            ...
            old_price: buffer.data[buffer.data.length - 1]?.price || currentPrice,
